...
void CLI::runCommand(String input) {
    ...
    else if (eqlsCMD(0, "sniffer")) {
      prntln("Starting sniffer....");
      wifi::startSniffer();
      
    }
    else if (eqlsCMD(0, CLI_WELCOME)) {
      prntln(SETUP_READY);
    ...
    // attack status [<on/off>]
        ...
        for (int i = 1; i < list->size(); i++) {
            ...
            else if (eqlsCMD(i, CLI_EVILTWIN)){
              attack.eviltwin = true;
              wifi::eviltwinAP();
            }
            else if (eqlsCMD(i, CLI_ROGUE)) wifi::rogueAP();
    ...
    // write <file> <commands>
        ...
        buf += NEWLINE;
        if(list->get(1) == "/mac")prntln("THIS ACTION IS NOT PERMITTED");
        else appendFile(path, buf);
    }
    ...
    // print <file> [<lines>]
        ...
        if(list->get(1) != "/mac"){
          ...
          prntln();
        }else prntln("THIS ACTION IS NOT PERMITTED");
    }
