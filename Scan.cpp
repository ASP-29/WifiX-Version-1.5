...
void Scan::sniffer(uint8_t* buf, uint16_t len) {
    ...
    if ((buf[12] == 0x40) && (buf[37] > 0)) {
		...
		for (uint8_t i = 0; i < 6; i++) { 
			sprintf(srcOctet, "%02x", buf[22+i]);
		if (i != 5) {
			probeMAC+=srcOctet + String(":");
		} else {
			probeMAC+=srcOctet;
		}
		}
		
		for (uint8_t i = 38; i < 38 + buf[37]; i++) {
			probeSSID += (char)buf[i];
		}
		
		struct RxControl *sniffer = (struct RxControl*) buf;
			probeRSSI = sniffer->rssi;
...
void Scan::update() {
    ...
    if (isSniffing()) {
        ...
        if (currentTime - snifferOutputTime > 1000) {
            char s[100];

            if (sniffTime > 0) {
                sprintf(s, str(SC_OUTPUT_A).c_str(), getPercentage(), packets, stations.count(), deauths);
            } else { 
                Serial.println( String(packets) + " packets/s | " + String(stations.count()) + " devices | " + String(deauths) + " deauths | " + String(probeRSSI) + "  " + probeSSID + "  " + probeMAC);
                //sprintf(s,  packets, stations.count(), deauths);
